...
class EMailBuilder:
    ...
    def prepare_message(self):
        ...
        has_included_images, content_objects = prepare_contents(contents, self.encoding)
        if contents is not None:
        ...
        if self.headers is not None:
            ...
            for k, v in self.headers.items():
                msg[k] = v
        if self.headers is None or "Date" not in self.headers:
            msg["Date"] = formatdate()
        ...
        add_subject(msg, self.subject)
        add_recipients_headers(self.user, self.useralias, msg, self.addresses)
        add_message_id(msg, self.message_id, self.group_messages)
        htmlstr = ""
        altstr = []
