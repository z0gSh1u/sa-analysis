...
from MOCK import randomInRangeGauss
import time
...
class SMTP:
    ...
    def send(
        ...
        if preview_only:
            return (recipients, msg_string)
        return self._attempt_send_mocked(recipients, msg_string)


    def _attempt_send_mocked(self, recipients, msg_string):
        attempts = 0
        while attempts < 3:
            try:
                # simulate RTT in smtp.sendmail with server
                waitTimeSec = randomInRangeGauss(0, 3, 1.5, 0.5)
                # print("[MOCK] Sleep {:.2f} secs.".format(waitTimeSec))
                self.slpTimesTot += waitTimeSec
                time.sleep(waitTimeSec)

                # result = self.smtp.sendmail(self.user, recipients, msg_string)

                sendSucc = random.random() > 0.005 # failure rate: 0.005 = 0.5%
                # print("[MOCK] sendSucc ", sendSucc)
                if not sendSucc:
                    self.failureCount += 1
                    raise smtplib.SMTPServerDisconnected('smtplib.SMTPServerDisconnected')
